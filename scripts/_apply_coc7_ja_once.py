...
def append_flow_aliases(text: str, additions: dict[str, list[str]]) -> str:
    start = text.index("alias:\n")
    end = text.index("st_show:\n", start)
    section = text[start:end]
    for canonical, aliases in additions.items():
        pattern = re.compile(rf"^(  {re.escape(canonical)}: \\[)(.*)(\\])$", re.M)
        match = pattern.search(section)
        if match is None:
            raise RuntimeError(f"alias line not found: {canonical}")
        values = match.group(2)
        for alias in aliases:
            if alias not in values:
                values += ", " + repr(alias).replace("'", '"')
        section = section[: match.start(2)] + values + section[match.end(2) :]
    if "  闪避:" not in section:
        section += '  闪避: [dodge, Dodge, "回避"]\n'
...
def main() -> None:
    ...
    text = append_flow_aliases(text, ALIASES)
